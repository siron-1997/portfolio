...
const createSectionAnimation = ({
    ...
    const handleReverseComplete = (type, animation = null) => {
         /* カメラ位置のとき */
        if (type === 'position') {
    ...
    if (!(element.id === 'controls')) { // Controls セクション以外
        ...
        const animation = gsap.timeline({ paused: true, duration: 0.6 }) // 変更されたカメラアングルを元のアングルに戻すアニメーション
        /* カメラ位置  */
...
export default function sectionsAnimation({
    ...
    /* pageHeader セクション */
    const pageHeaderAnimation = createSectionAnimation({
        element: pageHeader,
    ...
    /* introduction セクション */
    const introductionAnimation = createSectionAnimation({
    ...
    /* controls セクション */
    const controlsAnimation = createSectionAnimation({
        element: controls,
    ...
    /* クリーンアップ */
    return () => {
        pageHeaderAnimation.position.kill()
        pageHeaderAnimation.rotation.kill()
        introductionAnimation.position.kill()
        introductionAnimation.rotation.kill()
        controlsAnimation.position.kill()
        controlsAnimation.rotation.kill()
    }
}